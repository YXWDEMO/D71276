...
void option_init(void)
{
	//注意设置判断修改条件，防止出现一直修改复位
	 if((READ_BIT(FLASH->OPTR, OB_USER_SWD_NRST_MODE)!= OB_SWD_PB6_GPIO_PC0)|| (READ_BIT(FLASH->OPTR, OB_USER_BOR_EN)!= OB_BOR_ENABLE))
  {
  ...
  OBInitCfg.USERConfig = OB_BOR_ENABLE | OB_BOR_LEVEL_2p5_2p6 | OB_IWDG_SW | OB_IWDG_STOP_ACTIVE | OB_SWD_PB6_GPIO_PC0 ;
