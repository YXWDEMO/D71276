...
void Sleep_Gpio(void)
{
	...
	// LL_GPIO_SetOutputPin(Dis_Ic_PORT, Dis_Ic_PIN); // 输出高
	...
	//  SCL
	LL_GPIO_SetPinMode(SCL_PORT, SCL_PIN, LL_GPIO_MODE_INPUT); // 输出低
	LL_GPIO_SetPinPull(SCL_PORT, SCL_PIN, LL_GPIO_PULL_NO);

	LL_GPIO_SetPinMode(Dis_Ic_PORT, Dis_Ic_PIN, LL_GPIO_MODE_OUTPUT);
	LL_GPIO_SetPinSpeed(Dis_Ic_PORT, Dis_Ic_PIN, LL_GPIO_SPEED_FREQ_VERY_HIGH);
...
if(Key_Long_Time_Cnt<3000)
{
	...
	// 使能中断
	NVIC_SetPriority(KEY_1_ExtiIrqn, 0);
	NVIC_DisableIRQ(KEY_1_ExtiIrqn);
}
	...
	// LL_EXTI_InitTypeDef EXTI_InitStruct = {0};
	// EXTI_InitStruct.Line = SWITCH1_Exti;
	// EXTI_InitStruct.LineCommand = ENABLE;
	// EXTI_InitStruct.Mode = LL_EXTI_MODE_EVENT;
	// EXTI_InitStruct.Trigger = SWITCH1_ExtiMode;
	// LL_EXTI_Init(&EXTI_InitStruct);
	// // 触发源
	// LL_EXTI_SetEXTISource(SWITCH1_ExtiPort, SWITCH1_ExtiLine);
	// // 使能中断
	// NVIC_SetPriority(SWITCH1_ExtiIrqn, 0);
	// NVIC_EnableIRQ(SWITCH1_ExtiIrqn);
}
...
if(!KEY_IO)
    ...
    else
    {
      key_bit_temp1 = 0;
		...
		if ((usb_ad_bit_temp != usb_ad_bit_temp1)|| (mic_bit_temp != mic_bit_temp1) || (usb_bit_temp != usb_bit_temp1) || (key_bit_temp != key_bit_temp1) || (BatInfo.Usb_Full_Flag && (BatInfo.Bat_Volt < 4050)))
		{
			if (BatInfo.Usb_Full_Flag && (BatInfo.Bat_Volt < 4110))
			{
				// Bat_Leve=95;
				BatInfo.Usb_Rech_Flag=1;
				Dis_Ic_ON;
				CHRG_ENABLE;
				sysinit();
...
void Sleep(void)
{
	if (Dispinfo.Disp_Num ||!CHRG_IO|| OUTLOAD || (USBIN && (!(BatInfo.Usb_Full_Flag && (!BatInfo.Usb_Disp_Long_Cnt)))) || BatInfo.Bat_Leve_Delay_Cnt || Smg_Paoma_En_stop)
	{
...
void Gpio_Wake(void)
{
	...
	LL_GPIO_SetPinOutputType(SDA_PORT, SDA_PIN, LL_GPIO_OUTPUT_PUSHPULL);
	LL_GPIO_SetPinSpeed(SDA_PORT, SDA_PIN, LL_GPIO_SPEED_FREQ_VERY_HIGH);

	//SCL
	LL_GPIO_SetPinMode(SCL_PORT, SCL_PIN, LL_GPIO_MODE_OUTPUT);
	LL_GPIO_SetPinOutputType(SCL_PORT, SCL_PIN, LL_GPIO_OUTPUT_PUSHPULL);
	LL_GPIO_SetPinSpeed(SCL_PORT, SCL_PIN, LL_GPIO_SPEED_FREQ_VERY_HIGH);
